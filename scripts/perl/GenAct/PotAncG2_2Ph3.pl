...
my $databaseVG = 'ECM_Acts_VGPh3'; # 'ECM_Acts_VGPh2';
my $databasePA = 'ECM_Acts_PotAncPh3'; # 'ECM_Acts_PotAncPh2';
...
$sql = "SELECT `MS` FROM $source1 WHERE 1 ";
...
### CL2PotAnc: values summarised for all CL including fragments
		...
		#$dbh->do ("TRUNCATE `$databasePA`.`$table`; "); # truncate one time only: beginning of the script

		$dbh->do ("INSERT INTO `$databasePA`.`$table` 
						SELECT * FROM `$databaseVG`.`PotAncestors` ;");
		if ($DEBUG) {
			print "\n".__LINE__."INSERT INTO `$databasePA`.`$table` SELECT * FROM `$databaseVG`.`PotAncestors` ;";
		}
	}  #for (@wlist)
