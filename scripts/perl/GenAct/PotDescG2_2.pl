...
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0501PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0502PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0503PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0504PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0505PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0506PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0507PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0508PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0509PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0510PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0511PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0512PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0513PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0514PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0515PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0516PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0517PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0518PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0519PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0520PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0521PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0522PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0523PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0524PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0525PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0526PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0527PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0528PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0529PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAnc`.`0530PotDesc`");
...
{
	$ms=$_;
	if ($DEBUG) { print "current manuscript: $ms\n"; }
...
### Hier alle Kapitel, die nicht auskommentiert sind, noch einmal abfragen,
### bevor die Summierungstabelle dem Array hinzugef�gt wird.
		if ($writary[4] == 1 and $writary[5] == 1 and $writary[6] == 1 and $writary[7] == 1 and $writary[8] == 1 and $writary[9] == 1)
...
### CL2PotAnc/Desc_2: values summarised for all CL including fragments
		...
		if ($bsn==1) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0501PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==2) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0502PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==3) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0503PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==4) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0504PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==5) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0505PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==6) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0506PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==7) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0507PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==8) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0508PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==9) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0509PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==10) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0510PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==11) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0511PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==12) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0512PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==13) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0513PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==14) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0514PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==15) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0515PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==16) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0516PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==17) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0517PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==18) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0518PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==19) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0519PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==20) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0520PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==21) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0521PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==22) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0522PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==23) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0523PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==24) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0524PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==25) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0525PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==26) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0526PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==27) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0527PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==28) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0528PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==29) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0529PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
		if ($bsn==30) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAnc`.`0530PotDesc` 
						SELECT * FROM `ECM_Acts_VG`.`PotDescendants` ;");
		}
	}  #for (@wlist)
...
sub PotDesc {
	...
	if ($ms != 300350) {
		$sth = $dbh->prepare ("SELECT PASSAGES FROM `ECM_Acts_VG`.`$bs` WHERE (MS1='$ms' AND MS2='300350') OR (MS1='300350' AND MS2='$ms')"); # $bs ist z.B. NNGenTab_2
	} else {
		$sth = $dbh->prepare ("SELECT PASSAGES FROM `ECM_Acts_VG`.`$bs` WHERE MS1='$ms' AND MS2='306170'");
	}
...
#	}
	...
	$dbh->do ("DROP TABLE IF EXISTS `ECM_Acts_VG`.`TempTab`");

	$dbh->do ("CREATE TABLE `ECM_Acts_VG`.`TempTab` (
...
#	my @ary;
	...
	while (@ary = $sth->fetchrow_array())
	{	
		if ($ary[4]=$ms) { # ?Klaus: Was passiert hier?
	...
	$dbh->do ("DROP TABLE IF EXISTS `ECM_Acts_VG`.`PotDescendants`");

	$dbh->do("CREATE TABLE `ECM_Acts_VG`.`PotDescendants` (
 		`WITN1` varchar(10) NOT NULL default '',
 		`DIR` varchar(4) NOT NULL default '',
 		`WITN2` varchar(10) NOT NULL default '',
	 	`NR` int(3) NOT NULL default '0',
	...
	$dbh->do ("INSERT INTO `ECM_Acts_VG`.`PotDescendants` 
...
### number pot. descendants
	...
	$sth = $dbh->prepare ("SELECT * FROM `ECM_Acts_VG`.`PotDescendants`
...
##chance to reach high percentages of agreement and priority when compared with fuller mss.
			if ($count == 1) 
			{
				...
				$dbh->do ("UPDATE `ECM_Acts_VG`.`PotDescendants` SET	
							NR='$msa'
							WHERE WITN2='$ary3[2]';");
			} else {										
