...
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0501PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0502PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0503PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0504PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0505PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0506PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0507PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0508PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0509PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0510PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0511PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0512PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0513PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0514PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0515PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0516PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0517PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0518PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0519PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0520PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0521PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0522PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0523PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0524PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0525PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0526PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0527PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0528PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0529PotDesc`");
$dbh->do ("TRUNCATE TABLE `ECM_Acts_PotAncPh2`.`0530PotDesc`");
...
$sth = $dbh->prepare ("SELECT `MS` FROM $source1 WHERE `MS` < 400000"); # $source1 = ECM_Act_Mss.ActsMsList;
$sth->execute();
...
foreach $_ (@msl) 
{
	...
	for (@wlist) 
	{
		...
		if ($bs eq '0506GenTab_2') { $bsn=6; }
 		...
 		if ($bs eq '0510GenTab_2') { $bsn=11; }
 		if ($bs eq '0511GenTab_2') { $bsn=12; }
 		if ($bs eq '0512GenTab_2') { $bsn=13; }
 		if ($bs eq '0513GenTab_2') { $bsn=14; }
 		if ($bs eq '0514GenTab_2') { $bsn=15; }
 		if ($bs eq '0515GenTab_2') { $bsn=16; }
 		if ($bs eq '0516GenTab_2') { $bsn=10; }
...
### CL2PotAnc/Desc_2: values summarised for all CL including fragments
		...
		if ($bsn==1) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0501PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==2) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0502PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==3) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0503PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==4) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0504PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==5) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0505PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==6) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0506PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==7) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0507PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==8) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0508PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==9) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0509PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==10) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0510PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==11) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0511PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==12) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0512PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==13) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0513PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==14) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0514PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==15) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0515PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==16) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0516PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==17) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0517PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==18) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0518PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==19) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0519PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==20) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0520PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==21) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0521PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==22) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0522PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==23) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0523PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==24) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0524PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==25) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0525PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==26) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0526PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==27) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0527PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==28) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0528PotDesc` 
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");
		}
		if ($bsn==29) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0529PotDesc`
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");	
		}
		if ($bsn==30) {
			$dbh->do ("INSERT INTO `ECM_Acts_PotAncPh2`.`0530PotDesc`
						SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants` ;");	
		}		
...
sub PotDesc {
	...
	if ($ms != 309450) {
		$sth = $dbh->prepare ("SELECT PASSAGES FROM `ECM_Acts_VGPh2`.`$bs` WHERE (MS1='$ms' AND MS2='309450') OR (MS1='309450' AND MS2='$ms')"); # $bs ist z.B. NNGenTab_2
	} else {
		$sth = $dbh->prepare ("SELECT PASSAGES FROM `ECM_Acts_VGPh2`.`$bs` WHERE MS2='$ms' AND MS1='308080'");
	}
...
# The ranking number in PotDesc tables will remain the same accordingly.
	...
	$dbh->do ("DROP TABLE IF EXISTS `ECM_Acts_VGPh2`.`TempTab`");

	$dbh->do ("CREATE TABLE `ECM_Acts_VGPh2`.`TempTab` (
	...
	$dbh->do ("INSERT INTO `ECM_Acts_VGPh2`.`TempTab` 
					SELECT  * FROM `ECM_Acts_VGPh2`.`$bs` 
					WHERE (MS1='$ms' AND (DIR='-->' OR DIR=' ')) OR (MS2='$ms' AND (DIR='<--' OR DIR=' '))
					");


	$sth = $dbh->prepare ("SELECT WITN1, WITN2, DIR, MS1, MS2, W1FW2, W1TW2, PERC2, PERC3 FROM `ECM_Acts_VGPh2`.`TempTab`");
	$sth->execute();
...
#	my @ary;
	...
	{	
		if ($ary[4]=$ms) { 
			...
			$dbh->do ("UPDATE `ECM_Acts_VGPh2`.`TempTab` SET
					WITN2='$ary[0]',
					WITN1='$ary[1]',
					MS2='$ary[3]',
					MS1='$ary[4]',
		 ...
		 $dbh->do ("UPDATE `ECM_Acts_VGPh2`.`TempTab` SET DIR='<--'
	...
	$dbh->do ("DROP TABLE IF EXISTS `ECM_Acts_VGPh2`.`PotDescendants`");

	$dbh->do("CREATE TABLE `ECM_Acts_VGPh2`.`PotDescendants` (
 		`WITN1` varchar(10) NOT NULL default '',
 		`DIR` varchar(4) NOT NULL default '',
 		`WITN2` varchar(10) NOT NULL default '',
	 	`NR` int(3) NOT NULL default '0',
...
### number pot. descendants
	...
	$sth = $dbh->prepare ("SELECT * FROM `ECM_Acts_VGPh2`.`PotDescendants`
...
##chance to reach high percentages of agreement and priority when compared with fuller mss.
			if ($count == 1) 
			{
				...
				$dbh->do ("UPDATE `ECM_Acts_VGPh2`.`PotDescendants` SET	
							NR='$msa'
							WHERE WITN2='$ary3[2]';");
			} else {										
