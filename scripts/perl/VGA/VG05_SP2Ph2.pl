...
my $source1 = '`ECM_Acts_Mss`.`ActsMsList_2`'; # Phase 2
my $source2 = '`ECM_Acts_BasePh2`.`05'.$chnstr2.'WitGen`'; # WitGen!
my $source3 = '`ECM_Acts_BasePh2`.`05'.$chnstr2.'Rdg`';
...
foreach $ms(@msl)
{
	my $target = '`ECM_Acts_Sp_'.$chnstr2.'Ph2`.`Acts'.$chnstr2.'_'.$ms.'`';

# Tabelle lšschen, falls sie bereits existiert
	$dbh->do ("DROP TABLE IF EXISTS $target;");

# Tabelle neu anlegen, inklusive der Felder fŸr die genealogischen Informationen: Q1 + Q2
	 ...
	 `LABNEU` varchar(2) collate utf8_unicode_ci default NULL,
     `Q1` varchar(2) collate utf8_unicode_ci default NULL,
     `Q2` varchar(2) collate utf8_unicode_ci default NULL,
     KEY `ANFADR` (`ANFADR`,`ENDADR`)
	  ) ENGINE  = InnoDB  DEFAULT CHARSET  = utf8 COLLATE  = utf8_unicode_ci;");

	$dbh->do ("INSERT INTO $target SELECT ANFADR, ENDADR, LABEZ, HSNR, LABNEU, Q1, Q2 FROM $source2 WHERE HSNR = $ms;");
