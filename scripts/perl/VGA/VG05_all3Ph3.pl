...
my $target = '`ECM_Acts_VGPh3`.`VG05_all`';
...
foreach $ms(@msl)
{
	my $source2 = '`ECM_Acts_Sp_'.$chnstr2.'Ph3`.`Acts'.$chnstr2.'_'.$ms.'`';
	...
	foreach $cpms(@cpmsl)
	{
		my $source3 = '`ECM_Acts_Sp_'.$chnstr2.'Ph3`.`Acts'.$chnstr2.'_'.$cpms.'`';
		if ($DEBUG) { print "source3: ".$source3."\n"; }
		$gtxt = 0;
		$ueges = 0;
		
		my $sql = "SELECT ANFADR, ENDADR, LABEZ FROM $source2";	
