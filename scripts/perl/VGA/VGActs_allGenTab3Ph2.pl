...
my $source1 = '`ECM_Acts_Mss`.`ActsMsList_2`'; # unverŠndert immer ECM_Acts_Mss.ActsMsList - wer hat wo Text?
...
my $target = '`ECM_Acts_VGPh2`.`'.$book.$chnstr.'GenTab_2`'; # z.B. ECM_Acts_VG.0502GenTab_2
...
foreach $ms(@msl)
{
	my $source2 = '`ECM_Acts_Sp_'.$chnstr.'Ph2`.`Acts'.$chnstr.'_'.$ms.'`'; # z.B. ECM_Acts_Sp_01.Acts01_100740
	...
	foreach $cpms(@cpmsl)
	{
		my $source3 = '`ECM_Acts_Sp_'.$chnstr.'Ph2`.`Acts'.$chnstr.'_'.$cpms.'`'; # z.B. ECM_Acts_Sp_01.Acts01_317040
