...
###Chapter Number eintragen bzw. aus Kommandozeilenparameter Ÿbernehmen###
my $chn = $ARGV[0];
my $chnstr = '';
if ($chn < 10)
{
...
my $source  = '`ECM_Acts_CBGM`.`Acts'.$chnstr.'att`'; #'`ECM_Acts_CBGMPh2`.`Acts'.$chnstr.'att_2`';
my $target1 = '`VarGenAtt_Act`.`LocStemEdAct'.$chnstr.'`'; #'`VarGenAtt_ActPh2`.`LocStemEdAct'.$chnstr.'`';
my $target2 = '`VarGenAtt_Act`.`RdgAct'.$chnstr.'`'; #'`VarGenAtt_ActPh2`.`RdgAct'.$chnstr.'`';
my $target3 = '`VarGenAtt_Act`.`VarGenAttAct'.$chnstr.'`'; #'`VarGenAtt_ActPh2`.`VarGenAttAct'.$chnstr.'`';
...
while(@res = $sth->fetchrow_array())
{
    ...
    # is not possible here! All witness have to be copied
    ...
    $cmd .= "from `ECM_Acts_CBGM`.`Acts".$chn."att` ";
    ...
    {
        $cmd  = "insert into `VarGenAtt_Act`.`VarGenAttAct$chn` ";
    ...
    while(@res2 = $sth2->fetchrow_array())
    {
        ...
        $cmd  = "insert into VarGenAtt_Act.RdgAct".$chn;
