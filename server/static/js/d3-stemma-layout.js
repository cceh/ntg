...
define(['jquery', 'd3', 'd3-common', 'lodash'], function ($, d3, d3_common, _) {
    /**
    ...
    function load_dot(url) {
        ...
        d3_common.dot(url, function (graph) {
            ...
            svg.transition('svg').duration(300).attr('height', instance.bbox.height).attr('width', instance.bbox.width).transition().duration(300).style('opacity', 1.0);
            ...
            var subgraph = sg.selectAll('.subgraph').data(_.map(graph.subgraphs, 'attrs.graph.attrs')).enter();

            subgraph.append('rect').attr('x', function (d) {
                return d.bbox.x;
            }).attr('y', function (d) {
                return d.bbox.y;
            ...
            subgraph.append('text').attr('class', 'subgraph')
            ...
            var link = lg.selectAll('.link').data(graph.edges).enter();

            link.filter(function (d) {
                return d.attrs && d.attrs.head_lp;
            }).each(function (d) {
            ...
            }).attr('marker-end', 'url(#' + instance.id_prefix + 'triangle)').attr('d', function (d) {
                return d3_common.parse_path_svg(d.attrs.pos);
            });

            link.filter(function (d) {
                return d.attrs && d.attrs.head_lp && d.attrs.headlabel;
            }).append('text').attr('data-labez', function (d) {
                return graph.nodes[d.elems[0].id].attrs.labez;
            }).attr('class', function (d) {
                return 'link bg_labez ' + instance.id_prefix + 'sid-' + d.elems[0].id + ' ' + instance.id_prefix + 'tid-' + d.elems[1].id;
            }).attr('x', function (d) {
                return d.attrs.head_lp.x;
            }).attr('y', function (d) {
                return d.attrs.head_lp.y;
            }).text(function (d) {
                return d.attrs.headlabel;
            });
            ...
            var node = ng.selectAll('g.node').data(_.map(graph.nodes, 'attrs')).enter();
            ...
            var groups = node.append('g').attr('data-ms-id', function (d) {
                return d.ms_id;
            }).attr('data-label', function (d) {
                return d.label ? d.label : d.id;
            }).attr('class', function (d) {
                return 'node node-' + (d.children ? 'internal' : 'leaf') + (d.clickable ? ' clickable' : '') + (d.draggable ? ' draggable' : '') + (d.droptarget ? ' droptarget' : '');
            ...
            groups.append('ellipse').attr('class', 'node fg_labez bg_clique').attr('data-labez', function (d) {
                return d.labez;
            }).attr('data-clique', function (d) {
                return d.clique;
            }).attr('data-labez-clique', function (d) {
                return d.labez_clique;
            }).attr('rx', function (d) {
            ...
            groups.append('text').attr('class', 'node').text(function (d) {
                return d.label ? d.label : d.id;
            });
...
//# sourceMappingURL=d3-stemma-layout.js.map