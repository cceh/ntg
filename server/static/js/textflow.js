...
define(['jquery', 'lodash', 'tools', 'toolbar', 'css!textflow-css'], function ($, _, tools, toolbar) {
    function changed() {
        // currently unused
    ...
    function load_passage(passage) {
        ...
        var params = ['labez', 'connectivity', 'range', 'include', 'fragments', 'mode', 'hyp_a', 'var_only', 'width', 'fontsize', 'cliques'];

        // reset dropdown selection if reading is not in new passage
        if (!instance.data.global && _.findIndex(passage.readings, function (o) {
            return o[0] === instance.data.labez;
        }) === -1) {
        ...
        var p1 = toolbar.load_labez_dropdown(instance.$toolbar.find('div.toolbar-labez'), passage, 'labez', [], []);
        var p2 = toolbar.load_labez_dropdown(instance.$toolbar.find('div.toolbar-hyp_a'), passage, 'hyp_a', [['A', 'A']], []);
        var p3 = toolbar.load_range_dropdown(instance.$toolbar.find('div.toolbar-range'), passage, 'range', [{ 'range': 'This', 'value': 'x' }], []);

        return $.when(p1, p2, p3).done(function () {
        ...
        xhr.done(function (json) {
            ...
            _.forEach(cliques, function (value) {
                ...
                if (data.labez_new === data.labez_old && data.clique_new !== data.clique_old) {
                    menu.append($(tools.format('<tr data-action="move-manuscripts" ' + 'data-labez_old="{labez_old}" data-clique_old="{clique_old}" ' + 'data-labez_new="{labez_new}" data-clique_new="{clique_new}">' + '<td class="bg_labez" data-labez="{labez_old}"></td>' + '<td>Move subtree to {labez_new}{clique_new}</td>' + '</tr>', data)));
                }
            });
            ...
            menu.menu({
                'select': function select(ev, ui) {
                    ...
                    var xhr2 = $.getJSON('stemma-edit/' + passage.pass_id,
