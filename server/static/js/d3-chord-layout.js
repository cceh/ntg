...
define(['jquery', 'lodash', 'd3', 'd3-common'], function ($, _, d3, d3_common) {
    /**
    ...
    function load_dot(url) {
        ...
        svg.selectAll('g').transition().duration(300).style('opacity', 0.0).remove();
        var deferred = new $.Deferred();

        d3_common.dot(url, function (graph) {
            // copy subgraph nodes into the main graph
            _.forEach(graph.subgraphs, function (subgraph) {
                var subgraph_nodes = _.keyBy(_.filter(subgraph.stmts, function (o) {
                    return o.type === 'node';
                }), 'id');
                _.assign(graph.nodes, subgraph_nodes); // copy
                subgraph.attrs = _.keyBy(_.filter(subgraph.stmts, function (o) {
                    return o.type === 'attr';
                }), 'attrType');
            });
            ...
            _.forEach(graph.subgraphs, function (subgraph) {
                ...
                var subgraph_nodes = _.filter(subgraph.stmts, function (o) {
                    return o.type === 'node';
                });
            ...
            root.sort(function (a, b) {
                ...
                if (attrs_a.labez === 'a') {
                    ...
                    return (attrs_b.other_labez + attrs_a.hsnr).localeCompare(attrs_a.other_labez + attrs_b.hsnr);
                }
                // order by other_labez DECS, other_hsnr DESC
                return (attrs_b.other_labez + attrs_b.other_hsnr).localeCompare(attrs_a.other_labez + attrs_a.other_hsnr);
            });
            ...
            var radius = 1.2 * Math.max((n_nodes * node_height_px + n_groups * node_height_px / 2) / (2 * Math.PI), 2 * node_height_px);
            var label_radius = radius + 1.5 * node_width_px;

            d3.cluster().size([360, radius]).separation(function (a, b) {
            ...
            var ng = g.append('g').attr('class', 'nodes').attr('transform', 'rotate(-90)'); // put origin at 12 hours

            ng.selectAll('g.group').data(root.children).enter().append('g').attr('class', 'group').attr('transform', function (d) {
                return 'rotate(' + d.x + ') translate(' + label_radius + ') rotate (' + (90 - d.x) + ')';
            }).append('text').attr('class', 'group').text(function (d) {
                return d.data.attrs.graph.attrs.label;
            });

            var node = ng.selectAll('g.node').data(root.leaves()).enter().append('g').attr('data-ms-id', function (d) {
                return d.data.attrs.ms_id;
            }).attr('class', 'node node-leaf').attr('transform', function (d) {
                return 'rotate(' + d.x + ') translate(' + radius + ')';
            });

            node.append('ellipse').attr('class', 'node fg_labez bg_clique').attr('data-labez', function (d) {
                return d.data.attrs.labez;
            }).attr('data-clique', function (d) {
                return d.data.attrs.clique;
            }).attr('data-labez-clique', function (d) {
                return d.data.attrs.labez_clique;
            }).attr('rx', function (d) {
            ...
            node.append('text').attr('class', 'node').attr('transform', function (d) {
                return d.x < 180 ? null : 'rotate(180)';
            }).text(function (d) {
                return d.data.attrs.hs;
            });
            ...
            var link = lg.selectAll('path.link').data(graph.edges).enter().each(function (d) {
            ...
            link.append('path').attr('id', function (d) {
                return d.id;
            }).attr('data-labez', function (d) {
                return graph.nodes[d.elems[0].id].attrs.labez;
            }).attr('class', function (d) {
                return 'link fg_labez ' + instance.id_prefix + 'sid-' + d.elems[0].id + ' ' + instance.id_prefix + 'tid-' + d.elems[1].id + (/dashed/.test(d.attrs.style) ? ' dashed' : '');
            ...
            svg.transition('svg').duration(300).attr('height', bbox.height).attr('width', bbox.width).transition().duration(300).style('opacity', 1.0);
...
//# sourceMappingURL=d3-chord-layout.js.map